...
bool Clause::isPositiveLiteral(Int literal) {
  assert(literal != 0);
  return literal > 0;
}

Int Clause::getVar(Int literal) {
  assert(literal != 0);
  return abs(literal);
}
...
void Label::addNumber(Int i) {
  push_back(i);
  sort(begin(), end(), std::greater<Int>());
}

/* class Cnf ================================================================ */

string Cnf::helpWeightFormat() {
  string s = "weight format in CNF file: ";
  for (auto it = WEIGHT_FORMATS.begin(); it != WEIGHT_FORMATS.end(); it++) {
    s += it->first + "/" + it->second;
    if (next(it) != WEIGHT_FORMATS.end()) {
      s += ", ";
    }
  }
  return s + "; string";
...
void Cnf::printLiteralWeights() const {
  ...
  for (Int var = 1; var <= declaredVarCount; var++) {
    cout << "c  " << right << setw(10) << var << ": " << literalWeights.at(var) << "\n";
    cout << "c  " << right << setw(10) << -var << ": " << literalWeights.at(-var) << "\n";
  }
}
  ...
  for (const Clause& clause : clauses) {
    for (auto literal1 = clause.begin(); literal1 != clause.end(); literal1++) {
      for (auto literal2 = next(literal1); literal2 != clause.end(); literal2++) {
        Int var1 = Clause::getVar(*literal1);
        Int var2 = Clause::getVar(*literal2);
...
vector<Int> Cnf::getMostClausesVarOrder() const {
  std::multimap<Int, Int, std::greater<Int>> m; // clause count |-> var
...
vector<Int> Cnf::getLexmVarOrder() const {
  ...
  for (Int i = apparentVars.size(); i > 0; i--) {
    ...
    Graph subgraph = getGaifmanGraph(); // will only contain v, w, and unnumbered vertices whose labels are less than w's label
...
Cnf::Cnf(string filePath) {
  ...
  Int minic2dWeightsLineIndex = MIN_INT;
  ...
  while (getline(inputFileStream, line)) {
    ...
    if (words.empty()) {
      ...
      string cnfKey = weightFormat == PROJECTED ? "pcnf" : (weightFormat == WEIGHTED ? "wcnf" : "cnf");
      string cnfWord = words.at(1);
      if (cnfKey != cnfWord) {
        cout << WARNING << "expected '" << cnfKey << "', found '" << cnfWord << "' | line " << lineIndex << "\n";
      }

      declaredVarCount = stoll(words.at(2));
      declaredClauseCount = stoll(words.at(3));
    }
    else if (words.front() == "vp") { // additive-var line
      if (weightFormat == PROJECTED) {
      ...
      if (weightFormat == MINIC2D && words.size() > 1 && words.at(1) == WEIGHTS_WORD) { // MINIC2D weights line
        if (problemLineIndex == MIN_INT) {
          throw MyError("no problem line before MINIC2D weights | line ", lineIndex);
        }
        if (minic2dWeightsLineIndex != MIN_INT) {
          throw MyError("multiple MINIC2D weights lines: ", minic2dWeightsLineIndex, " and ", lineIndex);
        }
        minic2dWeightsLineIndex = lineIndex;

        if (words.size() != 2 + declaredVarCount * 2) {
          throw MyError("wrong number of MINIC2D literal weights | line ", lineIndex);
        }

        for (Int var = 1; var <= declaredVarCount; var++) {
          literalWeights[var] = Number(words.at(var * 2));
          literalWeights[-var] = Number(words.at(var * 2 + 1));
        }
      }
    }
    else if (words.front() == WEIGHT_WORD) {
      if (weightFormat != UNWEIGHTED) {
        if (problemLineIndex == MIN_INT) {
          throw MyError("no problem line before weight | line ", lineIndex);
        }

        if (weightFormat == CACHET && (words.size() == 3 || words.size() == 4 && words.at(3) == LINE_END_WORD)) {
          Int var = stoll(words.at(1));
          if (var <= 0 || var > declaredVarCount) {
            throw MyError("var '", var, "' inconsistent with declared var count '", declaredVarCount, "' | line ", lineIndex);
          }
          literalWeights[var] = Number(words.at(2));
        }
        else if ((weightFormat == WEIGHTED || weightFormat == PROJECTED) && (words.size() == 3 || words.size() == 4 && words.at(3) == LINE_END_WORD)) {
          Int literal = stoll(words.at(1));
          ...
          throw MyError("weight inconsistent with weight format ", WEIGHT_FORMATS.at(weightFormat), " | line ", lineIndex);
        }
      }
    }
    else { // clause line
      ...
      for (Int i = 0; i < words.size(); i++) {
        ...
        if (num == 0) {
          ...
          if (clause.empty()) {
            throw MyError("empty clause | line ", lineIndex);
          }
  ...
  assert(("CNF formula must be nonempty", !clauses.empty()));
  ...
  if (weightFormat == UNWEIGHTED) { // populates literalWeights with 1s
    for (Int var = 1; var <= declaredVarCount; var++) {
      literalWeights[var] = Number("1");
      literalWeights[-var] = Number("1");
    }
  }
  else if (weightFormat == MINIC2D) {
    assert(minic2dWeightsLineIndex != MIN_INT);
  }
  else if (weightFormat == CACHET) { // completes literalWeights
    for (Int var = 1; var <= declaredVarCount; var++) {
      Number varWeight("0.5"); // default
      if (literalWeights.contains(var)) {
        varWeight = literalWeights.at(var);
      }

      Number negativeLiteralWeight = Number("1") - varWeight;
      if (varWeight == Number("-1")) {
        varWeight = Number("1");
        negativeLiteralWeight = Number("1");
      }
  ...
  else { // completes literalWeights for WEIGHTED and PROJECTED
    for (Int var = 1; var <= declaredVarCount; var++) {
      if (!literalWeights.contains(var)) {
        literalWeights[var] = Number("1");
      }
...
vector<Int> JoinNonterminal::getBiggestNodeVarOrder() const {
  ...
  std::multimap<size_t, Int, std::greater<size_t>> m; // size |-> var
  for (pair<Int, size_t> varAndSize : varSizes) {
    m.insert({varAndSize.second, varAndSize.first});
  }

  vector<Int> varOrder;
  for (pair<size_t, Int> sizeAndVar : m) {
    varOrder.push_back(sizeAndVar.second);
  }
  return varOrder;
}
...
vector<Assignment> JoinNonterminal::getAdditiveAssignments(Int varOrderHeuristic, Int threads) const {
  TimePoint orderStart = util::getTimePoint();
  vector<Int> varOrder = getVarOrder(varOrderHeuristic);
  if (verbosityLevel > 0) {
    util::printRow("sliceVarSeconds", util::getSeconds(orderStart));
  }

  TimePoint assignmentsStart = util::getTimePoint();
  ...
  for (Int i = 0, assignedVars = 0; i < varOrder.size() && assignedVars < floor(log2(threads)); i++) {
  ...
  if (verbosityLevel > 0) {
    util::printRow("sliceAssignmentsSeconds", util::getSeconds(assignmentsStart));
  }
  return assignments;
}
