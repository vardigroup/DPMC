...
enum class VarOrderingHeuristic {
  ...
  APPEARANCE, DECLARATION, RANDOM, MCS, LEXP, LEXM, MIN_FILL
};
...
namespace util {
  ...
  Int getPositiveIntSeconds(Float seconds); // `alarm` and `setitimer` convert `seconds` to int, which can be 0 and disarm the timer
